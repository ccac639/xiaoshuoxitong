...
> 你重开项目时，无需把 TypeScript 逐行翻译成 Python——直接把文末第 9 节的「开发提示词」贴给新项目的 AI 助手，指明目标语言（建议 Python / FastAPI + asyncio），即可重生一套等价实现。
>
...
### ⚠️ 对「重开项目」的关键提示
**如果你新项目不是 Next.js（例如 Python / FastAPI、纯 Node 服务、Go 等），这个坑根本不存在**——直接用标准 HTTP 客户端（`httpx`、`requests`、原生 `fetch`、标准库 `https`）即可，中文天然正常。第 9 节的开发提示词已按「Python 项目无需 worker 绕过」来写，但你若**仍用 Next.js**，请保留「独立子进程 + base64 + 强制 identity」这套方案。
...
用 Python（FastAPI + asyncio + httpx）实现以下模块，要求语言无关的设计落地为 Python 代码。
...
> 重开项目时，**第 2 节（nativeFetch）通常不需要**——那是 Next.js 特定的坑。Python 项目直接用 httpx 即可。
